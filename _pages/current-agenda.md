...
Evolution produced a learning algorithm, the human brain, that adapts to radically different environments within a single lifetime. A person trained as an ML researcher could have been a civil engineer instead; the underlying cognitive machinery is the same. The brain achieves this through a handful of architectural innovations: a memory system for rapid, one-shot learning and selective retrieval; continuously maintained state; and a capacity to learn from sparse, costly feedback.
